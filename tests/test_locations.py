...
class LocationTestCase(BaseTestCase):
    ...
    @db_session
    def test_location_deleted(self):
        username1 = 'Dr.Dre'
        self._lm.add_user(username=username1, password='TheDocumentary')
        token_u1 = self._lm.login(username=username1, password='TheDocumentary')
        ...
        # Valid user delete a valid lociaton (loc1)
        self._lm.delete_location(username1, token_u1, 'Dres House')
        expected_locations = (loc2, loc3, loc4, loc5, loc6, loc7)
        res_locations = self._lm.get_all_locations(username1, token_u1)
        self.assertEqual(len(res_locations), 6, 'Unexpected location count')
        self.assertCountEqual(res_locations, expected_locations,
                                         'Unexpected location list returned')

        # Deleting a non-existing locaiton is ignored
        self._lm.delete_location(username1, token_u1, 'Non-existing')
        expected_locations = (loc2, loc3, loc4, loc5, loc6, loc7)
        res_locations = self._lm.get_all_locations(username1, token_u1)
        self.assertEqual(len(res_locations), 6, 'Unexpected location count')
        self.assertCountEqual(res_locations, expected_locations,
        ...
        # Make sure that a user can't delete another user's location
        # (user 1 deleting locaion loc2, created by user2)
        with self.assertRaises(AuthorizationError):
            self._lm.delete_location(username1, token_u1, 'Games House')
